...
BLOCK_H = AGL - MARGIN   # 3.5 m - anything shorter is simply flown over (read by foliage_state above, at call time)
...
# over BLOCK_H - is a TREE if any cell of it carries the solid bit, a STEM
...
def foliage_state(bake):
    ...
    cell standing over BLOCK_H, FOL_NONE elsewhere. None when the bake
    carries no kinds, and every blob is BUSH when it carries no bits."""
    kind = getattr(bake, "kind", None)
    if kind is None:
        return None
    tree = (kind == KIND_TREE) & (bake.obstacle > BLOCK_H)

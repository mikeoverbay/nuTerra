...
Public Module modMapSettings
    ...
    Private Iterator Function Fields() As IEnumerable(Of (Name As String, Reader As Func(Of Single), Writer As Action(Of Single)))
        ...
        Yield ("fog_density", Function() FOG_DENSITY, Sub(v) FOG_DENSITY = v)
        ...
        Yield ("tank_shadow", Function() B2F(TANK_SHADOW), Sub(v) TANK_SHADOW = F2B(v))
        Yield ("tank_cast_shadow", Function() B2F(TANK_CAST_SHADOW), Sub(v) TANK_CAST_SHADOW = F2B(v))
        Yield ("tank_shadow_debug", Function() B2F(TANK_SHADOW_DEBUG), Sub(v) TANK_SHADOW_DEBUG = F2B(v))
        Yield ("tank_shadow_range", Function() TANK_SHADOW_RANGE, Sub(v) TANK_SHADOW_RANGE = v)
        Yield ("tank_shadow_fade", Function() TANK_SHADOW_FADE, Sub(v) TANK_SHADOW_FADE = v)
        Yield ("tank_shadow_floor", Function() TANK_SHADOW_FLOOR, Sub(v) TANK_SHADOW_FLOOR = v)

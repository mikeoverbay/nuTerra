...
// Vertex half of the lamp pane GLOW pass - the second, additive draw that puts
// the panes into the FX buffer so they reach the bloom. Byte for byte the same
// as model_lamp.vert; only the fragment stage differs, and a program needs its
// own vertex stage.
// Identical to modelGlass.vert too - the three
// differ only in what the fragment stage does with the result - and kept
// separate rather than shared so changing one cannot move the other.

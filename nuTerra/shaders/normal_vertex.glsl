...
void main(void)
{
    ...
    n = normalize(vec3(projection * vec4(normalMatrix * vertexNormal, 0.0f)));
    t = normalize(vec3(projection * vec4(normalMatrix * vertexTangent, 0.0f)));
    b = normalize(vec3(projection * vec4(normalMatrix * vertexBinormal, 0.0f)));

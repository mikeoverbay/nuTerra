...
try_again:
    ...
    Public Sub ForceRender(Optional time As Single = 0.0)
        ...
        If map_scene IsNot Nothing Then bulb_placer.RenderView()
    ...
    ''' </summary>
    ''' <summary>The Light Bulb Placer - see BulbPlacer.vb. Owned here so it
    ''' outlives a map change; it re-reads its list when the map name moves.</summary>
    Public bulb_placer As New BulbPlacer()

    Private Sub draw_lamp_inspector()
        If Not SHOW_LAMP_VIEW Then Return
        bulb_placer.Draw(ClientSize, panel_origin(), panel_size(980, 600))
    End Sub
                ...
                If ImGui.CollapsingHeader("Shadow Mapping") Then
                    ...
                    If LAMP_SHADOW_ENABLED AndAlso MAP_LOADED AndAlso map_scene IsNot Nothing AndAlso
                       map_scene.lamp_shadow.ready Then
                        ...
                        If ImGui.SliderFloat("  lamp penumbra (texels)", v_ls, 0.0, 4.0) Then
                            LAMP_SHADOW_SOFT = v_ls
                        End If
                        If ImGui.IsItemHovered() Then
                            ImGui.SetTooltip("Softens the shadow edge with a 12 tap disc." & vbLf &
                                             "Measured in SHADOW MAP TEXELS, so the blur" & vbLf &
                                             "scales with the thing it is smoothing instead" & vbLf &
                                             "of erasing fine detail near the lamp." & vbLf &
                                             "0 is one fetch and a hard edge - the A/B.")
                        End If
                    End If
